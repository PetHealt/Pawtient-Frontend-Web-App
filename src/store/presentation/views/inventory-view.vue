...
const confirmDelete = async (id) => {
  if (confirm("¿Estás seguro de que deseas eliminar este insumo?")) {
    // Asegúrate de tener este método en tu storeStore
  ...
  <div class="p-4 lg:p-6">
    <div class="clinic-card">
      <div class="flex flex-column md:flex-row justify-content-between align-items-start md:align-items-center mb-5 gap-3">
        <div>
          <h1 class="text-3xl font-bold m-0 text-gray-800">Inventario de Almacén</h1>
          <p class="text-gray-500 m-0 mt-1">Controla el stock y precio de tus productos o medicamentos.</p>
        </div>
        <pv-button label="Nuevo Insumo" icon="pi pi-plus" class="btn-primary-clinic" @click="openNew" />
      </div>

      <pv-data-table :value="storeStore.products" :loading="storeStore.loading" stripedRows class="clinic-table">
        <template #empty> <div class="p-4 text-center text-gray-500">No hay insumos registrados en el almacén.</div> </template>

        <pv-column field="name" header="Insumo" sortable class="font-bold text-blue-600"></pv-column>
        <pv-column field="stock" header="Stock Actual">
          <template #body="slotProps">
            <!-- Pinta de rojo si hay poco stock (opcional, asumiendo que 5 es el límite) -->
            <span :class="{'text-red-500 font-bold': slotProps.data.stock <= 5}">
              {{ slotProps.data.stock }} unidades
            </span>
          </template>
        </pv-column>
        <pv-column field="price" header="Precio (S/)">
    ...
    >
      <template #header>
        <div class="flex align-items-center justify-content-between w-full pb-2">
          <div class="flex align-items-center gap-3">
            ...
            <h2 class="text-xl font-bold text-gray-800 m-0">{{ product.id ? 'Editar Insumo' : 'Nuevo Insumo' }}</h2>
          </div>
      ...
      <div class="grid formgrid pt-3">
        <div class="col-12 mb-4">
          <label class="block text-sm font-semibold text-gray-700 mb-2">Nombre del Insumo / Medicamento</label>
          <pv-input-text v-model="product.name" placeholder="Ej: Paracetamol 500mg" class="clinic-input" />
        </div>
      ...
      <template #footer>
        <div class="flex justify-content-end gap-3 pt-3">
          ...
          <pv-button label="Guardar Insumo" icon="pi pi-check" class="btn-primary-clinic" @click="handleSave" />
        </div>
      </template>
    </pv-dialog>
  </div>
...
.btn-primary-clinic {
  ...
  font-weight: 600 !important; padding: 0.75rem 1.5rem !important; transition: background-color 0.2s, transform 0.1s !important;
}
...
.clinic-dialog .p-inputtext, .clinic-dialog .p-inputnumber-input, .clinic-dialog .p-select, .clinic-dialog .p-dropdown {
  width: 100% !important; background-color: #f8fafc !important; border: 1px solid #cbd5e1 !important; color: #334155 !important;
  border-radius: 8px !important; padding: 0.8rem 1rem !important; font-size: 0.95rem !important; transition: all 0.2s ease !important; box-shadow: none !important;
}
.clinic-dialog .p-inputtext:focus, .clinic-dialog .p-select.p-focus, .clinic-dialog .p-dropdown.p-focus {
  background-color: #ffffff !important; border-color: #3b82f6 !important; box-shadow: 0 0 0 3px rgba(59, 130, 246, 0.15) !important; outline: none !important;
}
</style>